		...
		if err != nil {
			...
			if result {
				...
				if lastStatus[target.TargetURL] {
					...
					errorMsg := fmt.Sprintf("Target %s seems to be down. Recheck the target.", target.TargetURL)
					notify.SendEmailAlert("[OWTF-HEALTH-MONITOR]Error in target module", errorMsg)
